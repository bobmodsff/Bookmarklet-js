javascript:(()=> {
  const loginCorreto = "wagner", senhaCorreta = "modvipleo";
  ...
  if (inputLogin !== loginCorreto || inputSenha !== senhaCorreta)
    return void alert("❌ Login ou senha incorretos! Tente novamente.");
  ...
  const criarMenu = () => {
    ...
    e.id = "bobMenu";
    e.style = "position:fixed;top:100px;left:10px;background:linear-gradient(135deg,#111,#1a1a1a);color:white;padding:20px 20px 25px 20px;border-radius:15px;z-index:9999;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;max-width:90vw;border:2px solid;animation:glow 1s infinite alternate;box-shadow:0 0 20px rgba(0,255,255,0.5);user-select:none;";
    e.innerHTML = `<div class="rgb-fechar-x" title="Fechar menu" onclick="this.parentNode.remove()">✖</div><h3 style="text-align:center;margin-bottom:15px;font-size:20px">👑 BOB MODS FF - ESCOLA</h3>`;
    document.body.appendChild(e);

    const t = (text, _, action) => {
      const btn = document.createElement("button");
      btn.innerText = text;
      btn.className = "rgb-button";
      btn.onclick = action;
      e.appendChild(btn);
    };

    t("✅ HACK TAREFA", "", () => {
      alert("✅ SCRIPT BOB MODS FF EXECUTANDO COM SUCESSO...");
      fetch("https://corsproxy.io/?url=https://raw.githubusercontent.com/DarkModde/Dark-Scripts/refs/heads/main/TarefaResolver.js").then(e => e.text()).then(eval).catch(() => alert("❌ Erro ao carregar o script da Tarefa."));
    });

    t("📝 HACK REDAÇÃO", "", () => {
      alert("✅ SCRIPT BOB MODS FF EXECUTANDO COM SUCESSO...");
      fetch("https://corsproxy.io/?url=https://raw.githubusercontent.com/DarkModde/Dark-Scripts/refs/heads/main/RedaSP-IA.js").then(e => e.text()).then(eval).catch(() => alert("❌ Erro ao carregar o script de Redação."));
    });

    t("📚 KHAN HACK v.1 BOB", "", () => {
      alert("✅ SCRIPT BOB MODS FF EXECUTANDO COM SUCESSO...");
      fetch("https://corsproxy.io/?url=https://raw.githubusercontent.com/DarkModde/Dark-Scripts/refs/heads/main/KhanResolver.js").then(e => e.text()).then(eval).catch(() => alert("❌ Erro ao carregar o script do Khan Academy."));
    });

    t("❌ REMOVER O MOD MENU DOS SITE", "", () => {
      const menu = document.getElementById("bobMenu");
      const botao = document.querySelector(".flutuante");
      if (menu) menu.remove();
    ...
    aviso.innerText = "1. ⚠️ Se estiver na escola, use o X-VPN para funcionar\n2. 🏠 Em casa, use Wi-Fi normal\n\nUse com inteligência, jovem padawan!";
    e.appendChild(aviso);
    ...
    t("🌐 DISCORD DO BOB MODS", "", () => {
      window.open("https://discord.gg/fsYtn3z6", "_blank");
    });

    t("▶️ YOUTUBE BOB MODS FF", "", () => {
      window.open("https://youtube.com/@bobmodsff5508?si=wcsIGo7Oa8rKAWTX", "_blank");
    });

    const autor = document.createElement("p");
    autor.innerText = "Criador: Leonardo F.G";
    autor.style = "margin-top:15px;font-size:12px;color:#ccc;text-align:center";
    e.appendChild(autor);
  };
